...
@app.post("/api/job/submit_job_fields", responses={
    ...
    400: {"description": "Bad Request", "content": {"application/json": {"example": {"detail": "Field missing"}}}},
    500: {"description": "Internal Server Error", "content": {"application/json": {"example": {"detail": "Internal Server Error"}}}}
}, tags=["job"])
def submit_job_fields(response: SubmitJobFieldsResponseModel):
    try:
...
# FASTAPI file upload
def process_file(job_uuid: str, file: UploadFile = File(...)):
    try:
...
def purge_jobs(factory_uuid: str, status: str):
    try:
...
def mark_as_finished(job_uuid: str):
    try:
...
def mark_as_unfinished(job_uuid: str):
    try:
